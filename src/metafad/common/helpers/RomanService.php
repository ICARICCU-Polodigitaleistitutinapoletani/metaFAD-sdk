...
class metafad_common_helpers_RomanService
{
    ...
    static function extractRomanCentury($string)
    {
        if (strlen($string) == 4) {
            $integerCentury = (int)substr($string, 0, 2) + 1;
        } else {
            $integerCentury = (int)$string[0] + 1;
        }
        return self::integerToRomanic(strval($integerCentury));
    }
}
