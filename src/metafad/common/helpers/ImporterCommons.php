...
class metafad_common_helpers_ImporterCommons
{
    ...
    /**
     * Restituisce una versione normalizzata della data secolare indicata
     * @param integer|string $secolo
     * @return string
     */
    public static function normalizeCentury($century = '', $specifica = '')
    {
        if ($century === '' || $century === null) {
            return '';
        }
        $cent = metafad_common_helpers_RomanService::romanToInteger($century);
        if (!is_integer($cent) || $cent === 0) {
            return '';
        }
        switch ($specifica) {
            case 'Inizio':
                return (($cent - 1) * 100) . "0101-" . (($cent - 1) * 100 + 10) . "1231";
            case 'Metà':
                return (($cent - 1) * 100 + 46) . "0101-" . (($cent - 1) * 100 + 55) . "1231";
            case 'Fine':
                return (($cent - 1) * 100 + 90) . "0101-" . ($cent * 100) . "1231";
            case 'Prima metà':
                return (($cent - 1) * 100) . "0101-" . (($cent - 1) * 100 + 50) . "1231";
            case 'Seconda metà':
                return (($cent - 1) * 100 + 51) . "0101-" . ($cent * 100) . "1231";
            case 'Primo quarto':
                return (($cent - 1) * 100) . "0101-" . (($cent - 1) * 100 + 25) . "1231";
            case 'Secondo quarto':
                return (($cent - 1) * 100 + 26) . "0101-" . (($cent - 1) * 100 + 50) . "1231";
            case 'Terzo quarto':
                return (($cent - 1) * 100 + 51) . "0101-" . (($cent - 1) * 100 + 75) . "1231";
            case 'Ultimo quarto':
                return (($cent - 1) * 100 + 76) . "0101-" . ($cent * 100) . "1231";
            default:
    ...
    /**
     * Determina la tipologia della data a partire da un blocco cronologia.
     * Questo metodo deve essere chiamato solo dopo i metodi normalizeData e normalizeCentury, altrimenti darà un risultato errato.
     * @param $cronologia
     * @return string
     */
    public static function inferTipologiaData($cronologia)
    {
        $remoto_codifica = $cronologia->estremoRemoto_codificaData;
        $recente_codifica = $cronologia->estremoRecente_codificaData;
        if (!(empty($remoto_codifica) || empty($recente_codifica))) {
            return $remoto_codifica !== $recente_codifica ? 'data-intervallo' : 'data-singola';
        } elseif (!empty($recente_codifica)) {
            return 'data-intervallo';
        } else {
            return $remoto_codifica !== $recente_codifica ? 'data-aperta' : 'data-non-rilevabile';
        }
    }
    ...
    public static function inferToponimoFromIntestazione($intestazione)
    {
        ...
        $data->intestazione = $intestazione;
        $data->__model = "archivi.models.Toponimi";
    ...
    {
        if ($ar && $ar->getId()) {
            $docStruProxy = pinax_ObjectValues::get('org.pinax', 'application')->retrieveService('metafad.mag.models.proxy.DocStruProxy');
    ...
    {
        $arMAG = pinax_ObjectFactory::createModelIterator("metafad.mag.models.Model")
...
EOF
    ...
    /**
     * @param $identifier
     * @return mixed|null|pinax_dataAccessDoctrine_ActiveRecord
     */
    public static function getActiveRecordMAG($identifier)
    {
        $ar = pinax_ObjectFactory::createModelIterator("metafad.mag.models.Model")
            ->where("BIB_dc_identifier_index", $identifier)
            ->first();
        return $ar;
    }
    ...
    {
        return pinax_ObjectFactory::createModelIterator("metafad.strumag.models.Model")
