...
class metafad_common_importer_functions_transformers_ExtractFromNormal implements metafad_common_importer_functions_transformers_TransformerInterface
{
    ...
    public function __construct($params)
    {
        ...
        @$this->charToDelete = $params->charToDelete;
    }

    private function transformSingle($string)
    {
        $strings = explode($this->splitter, $string);
        if($this->charToDelete) {
            $strings = $this->deleteChar($strings, $this->charToDelete);
        }
        ...
        switch (strtoupper($this->type)) {
            ...
            case "RECENTE_CODIFICA":
                return $strings[1] ?: "";
            case "DATA_SECOLARE_REMOTO":
                return metafad_common_helpers_RomanService::extractRomanCentury($remoto[0]);
            case "DATA_SECOLARE_RECENTE":
                return metafad_common_helpers_RomanService::extractRomanCentury($recente[0]);
            default:
                return "";
        }
    }
    ...
    private function deleteChar($strings, $char)
    {
        $newStrings = [];
        foreach ($strings as $string) {
            $newStrings[] = str_replace($char, '', $string);
        }
        return $newStrings;
    }
}
