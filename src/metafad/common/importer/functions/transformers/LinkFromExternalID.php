...
class metafad_common_importer_functions_transformers_LinkFromExternalID implements metafad_common_importer_functions_transformers_TransformerInterface
{
    ...
    public function transformItems($array)
    {
         ...
         */
        $archProxy = pinax_ObjectFactory::createObject("archivi.models.proxy.ArchiviProxy");

        return array_map(
            function ($extId) use ($archProxy) {
                if(strpos($extId, 'rootCA##') !== false) {
                    $ar = pinax_ObjectFactory::createModel('archivi.models.ComplessoArchivistico');
                    $ar->load(explode('##', $extId)[1]);
                    return (object)array("id" => $ar->getId(), "text" => $ar->_denominazione);
                }
