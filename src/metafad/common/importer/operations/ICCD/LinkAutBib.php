...
class metafad_common_importer_operations_ICCD_LinkAutBib extends metafad_common_importer_operations_LinkedToRunner
{
    ...
    public function __construct(stdClass $params, metafad_common_importer_MainRunner $runnerRef)
    {
        ...
        $this->iccdFormProxy = $this->getOrSetDefault("iccdFormProxy", pinax_ObjectFactory::createObject('metafad.common.models.proxy.IccdFormProxy'));
        $this->uniqueIccdIdProxy = $this->getOrSetDefault("uniqueIccdIdProxy", pinax_ObjectFactory::createObject('metafad.gestioneDati.boards.models.proxy.UniqueIccdIdProxy'));
    }
    ...
    protected function getRefIDs($refs, $refHeader, $refNumber, $returnSigla, $modelName)
    {
        ...
        foreach ($refs as $ref) {
            $arr = (array)$ref;
            if (!empty($arr)) {
                $it = pinax_ObjectFactory::createModelIterator($modelName, 'all', array('filters' => array($refHeader => $ref->{$refHeader})));
                if ($it->count()) {
                    $ar = $it->current();
