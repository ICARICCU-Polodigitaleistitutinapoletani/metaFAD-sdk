...
class metafad_common_importer_operations_ICCD_BindMedia extends metafad_common_importer_operations_LinkedToRunner
{
    ...
    public function __construct(stdClass $params, metafad_common_importer_MainRunner $runnerRef)
    {
        ...
        $this->immftan = $this->getOrSetDefault("immftan", pinax_ObjectFactory::createObject($params->immftanClass, $params->immftanArg));
    }
    ...
    private function setImages(&$data)
    {
        ...
        if (is_array($images)){
            foreach ($images as $i => $image) {
                if ($image != null) {
                    $data->FTA[$i]->{'FTA-image'} = pinax_ObjectFactory::createObject("metafad_common_importer_utilities_ImagePlaceholder", $image);
                }
            }
        }
    }
