...
<div class="simpleSearch"><button class="btn btn-info btn-flat btn-add advancedSearchButton" data-action="advancedSearch"><i class="advancedSearchIcon fa fa-search"></i>Ricerca avanzata</button></div>
    <div class="advancedSearch">
        <div id="advancedSearchRepeaterContainer">
            ...
            <div class="moreElement clearfix" id="moreElement_0" data-id="0">
                <div class="adv-search-form-container">
                    <div id="search_0">
...
EOD;
        ...
        if ($searchFields) {
            foreach ($searchFields as $label => $field) {
                ...
                $advancedFilter .= pinax_helpers_Html::renderTag('option', $attributes, false, $label);
            }
        }
        $advancedFilter .=<<<EOD
                            </select>
            ...
            <div id="addFilterButton">
                <a id="addFilter" name="addFilter" class="fa fa-plus btn btn-info btn-flat btn-add addFilter"><span>Aggiungi filtro</span></a>
                <div class="btn-group">
                    ...
                    <input type="button" id="searchButton" name="searchButton" class="btn btn-info btn-flat btn-add" value="Cerca">
                </div>
            </div>
        </div>
    </div>
...
EOD;
    ...
    function render_html()
    {
        ...
        foreach ($this->columns as $column) {
            ...
            $headers .= pinax_helpers_Html::renderTag('th', $attributes, true, $content);
...
\$( function(){
    ...
    var table = \$('#$id').dataTable( {
        "sDom": "<'filter-row clearfix'<'filter-container'<'filter-box'f>>r><'table-container't><'row-fluid clearfix'<'select-record-section'><'filter-box select-page-length'l><'filter-box pull-left'i><'filter-box pull-right'p>>",
        ...
        "fnStateLoad": function (oSettings) {
            var fnStateLoad = JSON.parse(localStorage.getItem("$cookieName"));
            if(fnStateLoad)
            {
                if(!fnStateLoad.aaSorting)
                {
                    fnStateLoad.aaSorting = [];
                    fnStateLoad.abVisCols = [];
                }
            }
            return fnStateLoad;
        },
    ...
    $('$advancedFilter').appendTo("#{$id}_wrapper > div.filter-row .filter-container");
    ...
    $(document).ready( function () {
        ...
        $('body').on('click', '.js-result-select-all', function (e) {
            ...
            if (checked) {
                $.ajax({
                url: Pinax.ajaxUrl+'&controllerName=$selectAllController',
                data: {
        ...
        $('#addFilter').click( function() {
            ...
            var moreElement = '<div class="moreElement clearfix" id="moreElement_' + count1 + '" data-id="' + count1 + '"></div>';
            ...
            if (type == 'autocomplete') {
                ...
                container.find('input.searchValue').autocomplete({
                    ...
                    source: function( request, response ) {
                        $.ajax({
                            url: Pinax.ajaxUrl+'&controllerName=$autocompleteController',
                            data: {
                                instituteKey: '$filterInstitute',
                                model: '$recordClassName',
...
EOD;
    ...
    protected function getSearchFields($includeCheckbox=false)
    {
        ...
        if (strpos(',', $recordClassName) !== false) {
            $document = pinax_ObjectFactory::createModel($recordClassName);
