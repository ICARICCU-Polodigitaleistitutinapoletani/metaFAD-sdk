<?php
class metafad_thesaurus_views_components_DataGridAjax extends pinax_components_DataGridAjax
{
    private $code;
    ...
    function init()
    {
        ...
        $this->defineAttribute('joinField', false, 'thesaurusdetails_FK_thesaurus_id', COMPONENT_TYPE_STRING);
...
\$( function(){
    var table = \$('#$id').dataTable( {
        "sDom": "<'row-fluid filter-row clearfix'<'filter-box'l><'filter-box'f>r><'table-container't><'row-fluid clearfix'<'filter-box pull-left'i><'filter-box pull-right'p>>",
...
EOD;
    ...
    function process_ajax()
    {
        ...
        if ($pos = strpos($_SERVER['HTTP_REFERER'], 'edit')) {
            $id = intval(substr_replace($_SERVER['HTTP_REFERER'], '', 0, $pos + 5));
        }

        /*if(__Request::get('action') == 'index'){
            $it->select('DISTINCT iccd_theasaurs_code', 'iccd_theasaurs_module')->where('iccd_theasaurs_module != null');
        }*/ 
        ...
        if(__Request::get('action') == 'edit'){
            $it->where($this->getAttribute('joinField'), $id);
        }
        ...
        if ($this->getAttribute('dbDebug')) {
            pinax_dataAccessDoctrine_DataAccess::disableLogging(); die;
        }
