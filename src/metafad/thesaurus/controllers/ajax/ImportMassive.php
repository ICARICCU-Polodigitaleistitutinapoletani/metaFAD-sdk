...
class metafad_thesaurus_controllers_ajax_ImportMassive extends metafad_common_controllers_ajax_CommandAjax
{
    public function execute()
    {
        ...
        if ($_FILES) {
            ...
            pinax_importApplicationLib('PHPExcel/Classes/PHPExcel.php');
            ...
            foreach ($fileList as $key => $inputFileName) {
              ...
              $inputFileNameSplit = explode("/",$inputFileName);
              $thesaurusName = end($inputFileNameSplit);
              $dictionaryNames[] = $thesaurusName;
              $t = pinax_ObjectFactory::createModelIterator('metafad.thesaurus.models.Thesaurus')
              ...
              {
                $t = pinax_ObjectFactory::createModel('metafad.thesaurus.models.Thesaurus');
                $t->thesaurus_name = $thesaurusName;
                $t->thesaurus_code = $thesaurusName;
                $t->thesaurus_creationDate = new pinax_types_DateTime();
                $t->thesaurus_modificationDate = new pinax_types_DateTime();
                $idDizionario = $t->save();
              }
              ...
              if(__Request::get('Cancella_tutti') === 'true'){
                  ...
                  metafad_Metafad::logAction('metafad_thesaurus_controllers_ajax_ImportMassive delete id:'.$idDizionario, 'thesaurus');
                  foreach ($it as $ar) {
                      $ar->delete();
                  }
              }
              ...
              foreach ($arrayData as $key => $row) {
                  ...
                  $model = pinax_ObjectFactory::createModel('metafad.thesaurus.models.Details');
                  ...
                  if (__Request::get('Sostituisci_record') === 'true') {
                      ...
                      metafad_Metafad::logAction('metafad_thesaurus_controllers_ajax_ImportMassive delete key:'.$row[1], 'thesaurus');
                      foreach ($it as $ar) {
                          $ar->delete();
                      }
                  }
                  ...
                  $model->thesaurusdetails_creationDate = new pinax_types_DateTime();
                  $model->thesaurusdetails_modificationDate = new pinax_types_DateTime();
