function editDataGrid(data)
{
  ...
  $.ajax({
    url: Pinax.ajaxUrl + '&controllerName=metafad.thesaurus.controllers.ajax.SaveInput',
    type: 'POST',
    data: {
      id: id,
      val: val,
...
$( document ).ajaxComplete(function( event,request, settings ) {
  if(request.responseJSON !== undefined)
  {
    if(request.responseJSON.type == 'searchAjax' || request.responseJSON.result == 'level')
    {
      ...
      var model = '';
      var query = '';
      var proxy = 'metafad.thesaurus.models.proxy.ThesaurusDetailsProxy';
  ...
  $('.js-delete-row').on('click', function(e){
    ...
    else
    {
      ...
      var that = $(this);
      $.ajax({
        url: Pinax.ajaxUrl + '&controllerName=metafad.thesaurus.controllers.ajax.DeleteRecord',
        type: 'POST',
        data: {
          id: id
        },
  ...
  $('.js-add-row').unbind().click( function(e){
    ...
    else
    {
      ...
      $.ajax({
        url: Pinax.ajaxUrl + '&controllerName=metafad.thesaurus.controllers.ajax.AddRecord',
        type: 'POST',
        data: {
          id: id,
          value: value,
...
window.onload = function(){
  ...
  $('.js-pinaxcms-file').on('click', uploadFiles);
  $('.js-pinaxcms-importMassive').on('click', uploadFilesMassive);
  ...
  function uploadFiles(event) {
    ...
    {
      var urlParameter = '&controllerName=metafad.thesaurus.controllers.ajax.ImportDictionaryGE';
    }
    else if(importType == 'import3')
    {
      var urlParameter = '&controllerName=metafad.thesaurus.controllers.ajax.ImportDictionaryAL';
    }

    $.ajax({
      url: Pinax.ajaxUrl + urlParameter,
      type: 'POST',
      data: data,
      cache: false,
      dataType: 'json',
  ...
  function uploadFilesMassive(event) {
    ...
    {
      var urlParameter = '&controllerName=metafad.thesaurus.controllers.ajax.ImportMassiveGE';
    }
    else if(importType == 'import3')
    {
      var urlParameter = '&controllerName=metafad.thesaurus.controllers.ajax.ImportMassiveAL';
    }
    else if(importType == 'import4')
    {
      var urlParameter = '&controllerName=metafad.thesaurus.controllers.ajax.ImportMassiveAUTO';
    }
  ...
  $('.thesaurusModel').find('a').on('click',function(e){
    ...
    $.ajax({
      url: Pinax.ajaxUrl + '&controllerName=metafad.thesaurus.controllers.ajax.GetModule',
      type: 'POST',
      data: {id:data},
      dataType: 'json',
      success: function (data, textStatus, jqXHR) {
