...
class metafad_thesaurus_models_proxy_ThesaurusModulesProxy extends PinaxObject
{
    public function findTerm($fieldName, $subPageType, $query, $term, $proxyParams)
    {
        ...
        foreach ($modules as $key => $value) {
          if($term != '')
          {
            ...
            if (($value->isICCDModule === true || $value->hasDictionaries) && stripos($value->name,$term) !== false )
            {
              $result[] = array(
