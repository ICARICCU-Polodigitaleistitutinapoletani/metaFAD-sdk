...
class metafad_sbn_modules_sbnunimarc_views_components_Input extends pinax_components_HtmlFormElement
{
    /**
     * Init
     *
    ...
    function init()
    {
        ...
        $this->defineAttribute('authority', false, false, COMPONENT_TYPE_BOOLEAN);
    ...
    function render_html()
    {
        ...
        if ($this->_content && (count($this->_content)) != 1) {
        ...
        } else if(!$this->getAttribute('authority')){
            ...
            if(is_array($this->_content)){
                $array = $this->_content;
                if(strcmp ( substr($array[0], 0, 2) , '<A') == 0){
                    $newStr = str_replace('<', '', $array[0]);
                    $newStr = str_replace('>', '', $newStr);
                    $array[0] = $newStr;
                }
                if($attributes['id'] == 'personalName') {
                  $output = '<div ' . $this->_renderAttributes($attributes) . '/>' . htmlentities($array[0]) . '</div>';
                }
                else {
                  $output = '<div ' . $this->_renderAttributes($attributes) . '/>' . $array[0] . '</div>';
                }
