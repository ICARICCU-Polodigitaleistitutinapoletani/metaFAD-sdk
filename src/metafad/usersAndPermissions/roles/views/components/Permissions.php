<?php
class metafad_usersAndPermissions_roles_views_components_Permissions extends pinax_components_Input
{
    ...
    {
        $output = <<<EOD
        <div id="tableFixed" class="table-container table-fixed">
...
EOD;
        ...
        $permissionService = pinax_ObjectFactory::createObject(__Config::get('PERMISSION_CLASS'), __Config::get('PERMISSION_PARAMS'));
        ...
        <script type="text/javascript">
            $(function() {
                ...
                function resizeTable() {
                    const tableHeight = $('#tableFixed').height();
                    const deviceHeight = $(window).height();
                    const spacer = 60;
                    const spaceToRemove = $('#tableFixed').offset().top + $('.formButtons .content').height() + spacer;

                    $('#tableFixed').css('height', deviceHeight - spaceToRemove);
                }
            });
        </script>
EOD;
