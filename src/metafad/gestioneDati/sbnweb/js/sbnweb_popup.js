jQuery(document).ready(function () {
  ...
  Pinax.oop.declare("pinax.FormEdit.modalPageSBN", {
    ...
    receiveMessage: function (event)
    {
      ...
      var d = event.data;
      var bid = event.bid;
      if(d['type'] !== 'pinax.message.showSuccess' && d['type'] !== 'pinax.message.showError')
      {
        $('input#BID').val(bid);
        for (id in d)
        {
          ...
          if(type === 'AUT' && value !== undefined && importVID)
          {
            var vid = value;
            $.ajax({
              url: Pinax.ajaxUrl + '&controllerName=metafad.gestioneDati.sbnweb.controllers.ajax.GetAuthor',
              type: 'POST',
              data: {
  ...
  function checkVocabulary(element,value,type,inputType)
  {
    ...
    {
      var controllerName = '&controllerName=metafad.thesaurus.controllers.ajax.CheckVC';
    }
    else
    {
      var controllerName = '&controllerName=metafad.thesaurus.controllers.ajax.CheckVA';
    }
    var proxyParams = element.data('proxy_params');
    proxyParams = proxyParams.replace(/##/g, "\"");
    $.ajax({
      url: Pinax.ajaxUrl + controllerName,
      type: 'POST',
      data: {
        value: value,
        field: field,
