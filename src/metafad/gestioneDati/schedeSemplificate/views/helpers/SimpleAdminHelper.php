...
class metafad_gestioneDati_schedeSemplificate_views_helpers_SimpleAdminHelper extends PinaxObject
{
	public function getFields($id)
	{
		$ar = pinax_ObjectFactory::createModel('metafad.gestioneDati.schedeSemplificate.models.Model');
		$ar->load($id);
	...
	public function deleteFiles($id)
	{
		...
		{
			$moduleFile = str_replace('<pnx:Page pageType="'.$classPath.'.views.Admin_'.$name.'" adm:acl="*" id="'.$classPath.'_'.$name.'" value="{i18n:'.$ar->name.'}" />','',$moduleFile);
		}
		else
		{
			$moduleFile = str_replace('<pnx:Page pageType="' . $classPath . '.views.Admin_' . $name . '" adm:acl="*" id="' . $classPath . '_' . $name . '" parentId="gestione-dati/patrimonio" value="{i18n:' . $ar->name . '}" />', '', $moduleFile);
		}
