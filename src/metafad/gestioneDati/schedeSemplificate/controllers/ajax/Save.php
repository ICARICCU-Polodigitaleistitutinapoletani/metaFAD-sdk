<?php
class metafad_gestioneDati_schedeSemplificate_controllers_ajax_Save extends pinaxcms_contents_controllers_moduleEdit_ajax_Save
{
    public function execute($data, $draft=false)
    {
        ...
        if($isNew)
        {
          if(__Config::get('metafad.iccd.hasList'))
          {
            $moduleFile = str_replace('</pnx:Page>', '<pnx:Page pageType="' . $classPath . '.views.Admin_' . $name . '" adm:acl="*" id="' . $classPath . '_' . $name . '" value="{i18n:' . $decodeData->name . '}" /></pnx:Page>', $moduleFile);
          }
          else
          {
            $moduleFile = str_replace("/>';", '/><pnx:Page pageType="' . $classPath . '.views.Admin_' . $name . '" adm:acl="*" id="' . $classPath . '_' . $name . '" parentId="gestione-dati/patrimonio" value="{i18n:' . $decodeData->name . '}" />\';', $moduleFile);
          }
        }
        else
        {
          $moduleFile = str_replace('<pnx:Page pageType="'.$classPath.'.views.Admin_'.$oldName.'" adm:acl="*" id="'.$classPath.'_'.$oldName.'" value="{i18n:'.$oldDataLabel.'}" />',
                                    '<pnx:Page pageType="'.$classPath.'.views.Admin_'.$name.'" adm:acl="*" id="'.$classPath.'_'.$name.'" value="{i18n:'.$decodeData->name.'}" />',
                                    $moduleFile);
        }
        ...
        $adminFile = str_replace('<pnx:template name="form_fields">','<pnx:template name="form_fields"><pnx:Hidden id="simpleForm" value="'.$decodeData->__id.'"/>',$adminFile);
