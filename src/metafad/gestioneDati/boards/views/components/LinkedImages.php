...
class metafad_gestioneDati_boards_views_components_LinkedImages extends pinax_components_Component
{
    function init()
    {
    ...
    public function render($outputMode=NULL, $skipChilds=false)
    {
        ...
        $ar = pinax_ObjectFactory::createModelIterator($model.'.models.Model')
        ...
        if ($ar->FTA && $size > 0)
        {
          ...
          foreach ($ar->FTA as $key =>$FTA) {
            ...
            $orig = $dam->resizeStreamUrl(json_decode($FTA->{'FTA-image'})->id,'original', __Config::get('metafad.dam.maxResizeWidth'));
