<?php
class metafad_gestioneDati_boards_models_proxy_RelationsProxy extends PinaxObject
{
    public function processRelations($data)
    {
        ...
        $it = pinax_ObjectFactory::createModelIterator('metafad.gestioneDati.boards.models.Relations')
  ...
  public function saveRelation($data)
  {
    ...
    if($data->RV[0]->RVE)
    {
      foreach ($data->RV[0]->RVE as $r) {
          $arr = (array)$r;
          if (!empty($arr)) {
            $relation = pinax_ObjectFactory::createModel( 'metafad.gestioneDati.boards.models.ComplexRelations' );
    ...
    if($data->RV[0]->RSE)
    {
      foreach ($data->RV[0]->RSE as $r) {
          $arr = (array)$r;
          if (!empty($arr)) {
            $relation = pinax_ObjectFactory::createModel( 'metafad.gestioneDati.boards.models.Relations' );
  ...
  {
    $relation = pinax_ObjectFactory::createModelIterator( 'metafad.gestioneDati.boards.models.Relations' )
