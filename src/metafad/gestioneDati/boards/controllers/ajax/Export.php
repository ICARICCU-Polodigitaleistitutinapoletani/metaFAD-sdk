...
class metafad_gestioneDati_boards_controllers_ajax_Export extends metafad_common_controllers_ajax_CommandAjax
{
    public function execute()
    {
        ...
        $exportMets = (__Request::get('exportMets')) ?: '';
        $exportType = (__Request::get('exportType')) ?: '';

        $module = (__Request::get('pageId')) ?: '';

        if ($exportFormat === 'ead3' && count($_FILES) < 1) {
            $this->directOutput = true;
            return array('msg' => 'ATTENZIONE: caricare il file di configurazione');
        }
        ...
        if (file_exists($file['tmp_name'])) {
            ...
            try{
                ...
                $this->directOutput = true;
                return array('msg' => 'Il file ' . $file['name'] . ' ha un errore di sintassi oppure non è in formato xml.');
            }
            $fileConf = __Paths::getRealPath('CACHE') . md5(time() . $file['name']);
            if (!move_uploaded_file($file['tmp_name'], $fileConf)) {
                return array('msg' => 'Errore nel caricamento dei file');
            }
        }
        ...
        if (sizeof($schList) == 0 || $schList[0] == '') {
            ...
            $jobFactory->createJob(
                'metafad.exporter.helpers.Batch',
                array(
                    'format' => $exportFormat,
                    'arrayIds' => $schList,
                    'moduleName' => $module,
            ...
            $this->directOutput = true;
            $url = __Routing::makeUrl('link', array('pageId' => 'metafad.jobsReport'));
