<?php
class metafad_gestioneDati_boards_controllers_Edit extends metafad_common_controllers_Command
{
    public function execute($id, $templateID)
    {
        ...
        if ($id) {
            ...
            if ($data['BIB'][0]->__BIB) {
                $data['BIB'] = pinax_ObjectFactory::createObject('metafad.gestioneDati.boards.views.helpers.BibHelper')->getBibValues($data['BIB']);
            }
            if ($data['AUT'][0]->__AUT) {
                $data['AUT'] = pinax_ObjectFactory::createObject('metafad.gestioneDati.boards.views.helpers.AutHelper')->getAutValues($data['AUT']);
            }
            $data['__id'] = $id;
            ...
            $linkedImages = $this->view->getComponentById('linkedImages');
            if ($linkedImages) {
                $linkedImages->setAttribute('enabled', false);
            }
