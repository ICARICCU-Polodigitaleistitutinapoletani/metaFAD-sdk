<?php
class metafad_viewer_rest_controllers_Viewer extends pinax_rest_core_CommandRest
{
	...
	public function execute($id)
	{
		...
		$secondary = __Request::get('secondary');
		...
		if($type == 'sbn') {
			...
			if($record->load($id)){
				$ar = pinax_ObjectFactory::createModel($record->getType().'.models.Model');
				$ar->load($id);
			...
			$titleKey = current(explode(',',str_replace(array('_s','_t'),array('',''),end($solrDocument))));

			if(strpos($titleKey,',') !== false)
			{
				$titleSplit = explode(',',$titleKey);
				$titleKey = end($titleSplit);
			}
		...
		else if($type == 'archive')
		{
			...
			if($record->load($id)){
				...
				$this->dam = pinax_ObjectFactory::createObject('metafad.dam.services.ImportMedia', $instituteKey);
				$ar = pinax_ObjectFactory::createModel($record->document_type);
				$ar->load($id);
				$doc->licenses = $this->getGlobalLicenseDetail($ar->getRawData()->ecommerceLicenses);
				...
				if($record->linkedStruMag)
				{
					$null = null;
					if($secondary) {
						$strumagId = $record->secondaryStruMag[0]->relatedStruMag->id;
					} else {
	...
	function getGlobalLicenseDetail($licenses)
	{
		if($licenses)
		{
			...
			foreach ($licenses as $l) {
				$licenseProxy = pinax_ObjectFactory::createObject('metafad_ecommerce_licenses_models_proxy_LicensesProxy');
	...
	function iterateImage(&$physicalSTRU,$init,$num,$data,$type,$id,&$tot = null,$folder = null)
	{
		...
		foreach($data as $v)
		{
			...
			$image['thumbnail'] =  metafad_dam_Common::replaceUrl($this->dam->streamUrl($d->id,'thumbnail'));
			$image['title'] =  $d->title;
			$image['id'] =  $d->id;
			$image['type'] =  ($d->type)?:'IMAGE';
			$image['url'] = $this->dam->streamUrl($d->id,'original');
			$image['label'] =  ($d->label) ?: $d->title;
	...
	{
		$request = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', __Config::get('metafad.metaindice.detail.url'), 'POST', 'id='.$id, 'application/x-www-form-urlencoded');
