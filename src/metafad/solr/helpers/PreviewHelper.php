<?php
class metafad_solr_helpers_PreviewHelper extends PinaxObject
{
    ...
    public function getHtmlFromStructure($data, $structure, $allSubFields = null, $complex = false)
    {
        ...
        if ($data->{'FTA-image'} && array_key_exists('FTAX', $structure)) {
            $appoggioArray = array();
            $appoggioArray['FTA-image'] = array();
            foreach ($structure as $k => $v) {
                $appoggioArray[$k] = $v;
            }
            $structure = $appoggioArray;
            unset($appoggioArray);
        }
            ...
            if ($data->$key) {
                ...
                if (is_string($d) && $d) {
                    if ($key == 'FTA-image') {
                        ...
                        $dam = $viewerHelper->initializeDam($instituteKey);
                        $json = json_decode($d);
                        $imageUrl = metafad_dam_Common::replaceUrl($dam->streamUrl($json->id, 'thumbnail'));
                    ...
                    $htmlAppoggio = '';
                    foreach ($d as $k => $v) {
                        $htmlAppoggio .= '<div class="group-value">';
                        ...
                        foreach ($v as $field => $val) {
                            if (is_string($val) && $val) {
                                ...
                                if ($key == 'ROZ' || $key == 'RSE') {
                                    $key = ($key == 'RSE') ? 'RSEC' : $key;
                                    $htmlAppoggio .= $this->getHtmlLabelValue($key, $val->text);
    ...
    public function detailMapping($data, $elements = null)
    {
        $doc = new stdClass();
        ...
        $elements = ($elements) ?: json_decode(file_get_contents($modulePath));
                ...
                if (strpos($v, ':') !== false) {
                    ...
                    if ($path[0] == 'AUT') {
                        ...
                        if ($record->load($fieldValue[0]->__AUT->id)) {
                            foreach ($record->getRawData() as $key => $value) {
                                $path1 = $path[1];
                                if ($key == $path[1] && $fieldValue[0]->$path1 == null) {
                                    $fieldValue[0]->$path1 = $value;
                                }
                            }
                        }
                    }
                    if (!is_string($fieldValue) && $fieldValue != null) {
                        foreach ($fieldValue as $f) {
                            $path1 = $path[1];
                            $this->setValues($f->$path1, $doc->$label);
                        }
