...
class metafad_solr_helpers_MetaindiceHelper extends PinaxObject
{
    private $mappingFile;
    ...
    public function mapping($data, $type, $option = "commit", $solrData = null, $ecommerceInfo = null)
    {
        ...
        $ecommHelper = pinax_ObjectFactory::createObject('metafad_ecommerce_helpers_EcommerceToSolrHelper');
        ...
        if ($solrData == null) {
            $request = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $url . 'select', 'POST', 'q=id:' . $data->__id . '&wt=json', 'application/x-www-form-urlencoded');
        ...
        //Per abilitare il salvataggio in metaindice per un certo istituto, aggiungere la chiave insituto in config
        $instituteArray = json_decode(__Config::get("metafad.archive.saveMetaindice"));
        $instKey = metafad_usersAndPermissions_Common::getInstituteKey();
        if ($instituteArray && in_array($instKey, $instituteArray)) {
            $saveOnMetaindice = true;
        } else {
            $saveOnMetaindice = false;
        }
        if ($saveOnMetaindice) {
            $evt = array('type' => 'insertData', 'data' => array('data' => $docForSolr, 'option' => array(($option === "queue" ? "queue" : "commit") => true, 'url' => $metaindexUrl)));
            $this->dispatchEvent($evt);
            if (__Config::get('solr.core.metaindice_be')) {
                $metaindexUrl = __Config::get('metafad.solr.metaindiceBE.url');
            
                $docForSolr['url_t'] = $this->getEditingUrl($data); 
    ...
    private function getEditingUrl($data)
    {
        if(!$data->document_detail_status){
            $doc = json_decode($data->document);
            $statusEdit = ($doc->hasPublishedVersion) ? 'edit' : 'editDraft';
        }
        else{
            $statusEdit = ($data->document_detail_status == 'PUBLISHED') ? 'edit' : 'editDraft'; 
        }
        if($data->document_detail_FK_document_id){
            $id = $data->document_detail_FK_document_id;
        }
        else{
            $id = $data->__id;
        }
        return $data->pageId . '/' . $statusEdit . '/' . $id .'/';
    }

    /**
     * @param $data stdClass Dati da salvare/mappare
     */
    public function genericMapping($data, $option = null)
    {
        $metaindexUrl = __Config::get('metafad.solr.metaindice.url');

        $model = pinax_ObjectFactory::createModel($data->__model);
        if (method_exists($model, 'getGenericMappingIndication')) {
            $mapping = $model->getGenericMappingIndication();
        }
        ...
        if (!empty($mapping)) {
            ...
            foreach ($mapping as $key => $value) {
                if (strpos('_ss', $key) !== false) {
                    $docForSolr->$key = $data->$value;
                } else {
                    $docForSolr->$key = array($data->$value);
                }
            }

            //DATI PER IL DETTAGLIO
            $feHelper = pinax_ObjectFactory::createObject('metafad_solr_helpers_FeHelper');
            $moduleId = str_replace('.models.Model', '', $data->__model);
            $moduleService = __ObjectFactory::createObject('metafad.iccd.services.ModuleService');
            $elements = $moduleService->getElements($moduleId);
            $values = $feHelper->processElements($elements, $data);
            $docForSolr->data_s = json_encode($values);

            //Campo di sistema per gestione link in FE
            $docForSolr->is_generic_s = 'true';
