...
class metafad_solr_Listener extends PinaxObject
{
    ...
    function __construct()
    {
        ...
        $this->addEventListener('commitFe', $this);
    }
    ...
    public function insertRecordFE($evt)
    {
        ...
        $feHelper = pinax_ObjectFactory::createObject('metafad.solr.helpers.FeHelper');
        ...
        if ($solrModel['feMapping']) {
			...
			if(!$isAut)
			{
	            ...
	            if($data->RV)
	            {
	              if($data->RV[0]->RVE[0]->RVEL !== null || $data->RV[0]->RVE[0]->RVEL !== '')
	              {
                    $doc->level_s = $data->RV[0]->RVE[0]->RVEL;
                    $maskedSort = '000000000000'. $doc->level_s;
                    $doc->level_sort_s = substr($maskedSort,-10);
	              }
	            }
            ...
            //Check per eventuali relazioni
            //Spento, può tornare utile ma non serve così attualmente
            //$relations = (method_exists($feHelper, 'checkLevelRelation')) ? $feHelper->checkLevelRelation($data) : '';
            $relations = (method_exists($feHelper, 'getLinkSearchRelation')) ? $feHelper->getLinkSearchRelation($data) : '';
    ...
    public function commitFe($evt)
    {
        $data = $evt->data;
        $option = $data['option'];
        $this->sendRequest('commit', null, $option);
        $this->documents = array();
    }
    ...
    {
        return pinax_ObjectFactory::createObject('metafad.solr.helpers.BeHelper');
    }
    ...
    {
        return pinax_ObjectFactory::createModel($model);
