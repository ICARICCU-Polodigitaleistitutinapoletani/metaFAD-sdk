<?php
class metafad_dam_Bridge implements pinaxcms_mediaArchive_BridgeInterface
{
    protected $cmsCmpatibility;

    function __construct()
    {
        $this->cmsCmpatibility = __Config::get('metafad.dam.cmsIntegration')===true;
    }

    protected function getDamUrlFe()
    {
        return __Config::get('metafad.dam_fe.url') . '?instance=' . __Config::get('metafad.dam.instance');
    }

    protected function getDamUrl()
    {
        return metafad_dam_Common::getDamUrl() . '/';
    }
    ...
    {
        $r = pinax_ObjectFactory::createObject(
        ...
        if ($info['http_code'] == 200 && $body) {
            return pinax_ObjectFactory::createObject('metafad.dam.models.vo.MediaInfoVO', $body);
        }
