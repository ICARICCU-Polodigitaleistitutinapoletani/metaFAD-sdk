<?php
class metafad_dam_services_ImportMedia implements metafad_dam_services_ImportMediaInterface
{
	protected $damUrl;
	protected $damUrlLocal;
	protected $damInstance;

	/** var pinax_log_LogBase */
	protected $logger;

	public function __construct($damInstance = null, pinax_log_LogBase $logger = null)
	{
	...
	public function mediaExists($filePath)
	{
		...
		$result = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $this->damUrlLocal.'/exists/md5/'.$md5);
		$result->execute();
	...
	{
		$r = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $this->damUrlLocal.'/'.$type, 'POST', $mediaData, 'application/json');
	...
	public function streamUrl($id, $stream)
	{
		...
		if ($stream == 'original') {
			$url = $this->damUrl.'/resize/'.$id.'/original?h=*&w='.__Config::get('metafad.dam.maxResizeWidth');
		} else {
	...
	public function addMediaToContainer($magName, $medias, $cover, $tag = null)
	{
		...
		if($tag !== null && is_array($tag)) {
			$containerData['MainData']['tag'] = $tag;
		}
		$containerData['mediaId'] = reset(json_decode($medias)->addMedias);
		$container = $this->insertMedia(json_encode($containerData),'container');
		$id = $container->id;

		$r = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $this->damUrlLocal.'/container/'.$id.'/ContainedMedia', 'POST', $medias, 'application/json');
		$response = $r->execute();
	}

	public function deleteContainer($id, $removeContainedMedia=false)
	{
		$r = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $this->damUrlLocal.'/container/'.$id.'?removeContainedMedia='.($removeContainedMedia ? 'true' : 'false'), 'DELETE', [], 'application/json');
		$response = $r->execute();
	}

	public function insertBytestream($data,$id)
	{
		$r = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $this->damUrlLocal.'/media/'.$id.'/bytestream', 'POST', $data, 'application/json');
		$result = null;
		if ($r->execute()) {
			$info = $r->getResponseInfo();
			$body = json_decode($r->getResponseBody());
	...
	public function search($title)
	{
		...
		$r = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $this->damUrlLocal.'/search', 'POST', json_encode($params), 'application/json');
		$r->execute();
	...
	public function correctInstance($id)
	{
		...
		if($r->getResponseBody())
		{
			...
			$this->damUrl = metafad_dam_Common::getDamUrl($realInstance);
		}
	}
	...
	public function saveMediaDatastream($mediaId, $mediaData,$datastreamName,$datastreamId = null, $method = null)
	{
		...
		$r = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $this->damUrlLocal . '/media/' .$mediaId .'/datastream/'. $datastreamName .(($datastreamId)?'/'.$datastreamId:''), $method, $mediaData, 'application/json');
		$r->setTimeout(__Config::get('metafad.dam.timeout'));
		$response = $r->execute();
		if ($response) {
			$info = $r->getResponseInfo();
