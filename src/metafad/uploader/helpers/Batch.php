<?php
class metafad_uploader_helpers_Batch extends metafad_jobmanager_service_JobService
{
	public function run()
	{
	...
	private function checkStatus($message,$serviceStatus) {
		...
		while($response->status != 'finished')
		{
			...
			if($response->convertorStatus)
			{
				...
				$this->setMessage('Creazione pacchetto dati in corso, caricati correttamente su server '.$count.' record.');
				$this->save();
			}
