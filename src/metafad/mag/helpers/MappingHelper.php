<?php
class metafad_mag_helpers_MappingHelper extends PinaxObject
{
  ...
  public function getMapping($record,$data,$model,$id = null)
  {
    ...
    if($model == 'archivi.models.UnitaDocumentaria') {
      $this->archiviProxy = pinax_ObjectFactory::createObject("archivi.models.proxy.ArchiviProxy");
  ...
  private function getIccdCreator($record,$type)
  {
    ...
    if($record->AUT)
    {
      foreach ($record->AUT as $aut) {
        $autId = $aut->__AUT->id;
        $ar = pinax_ObjectFactory::createObject('pinax.dataAccessDoctrine.ActiveRecordDocument');
        if($ar->load($autId))
        {
  ...
  public function createMag($mapping,$importMode,$linkedModel,$formId,$createImgGroup = false)
  {
    ...
    if($importMode == 'substitute')
    {
      ...
      if($dc_identifier)
      {
        ...
        {
          foreach ($it as $ar) {
            $this->deleteMag($ar->document_id,'metafad.mag.models.Model');
          }
        }
      }
    }
    ...
    $cl = new stdClass();
    $cl->className = 'metafad.mag.models.Model';
    $cl->isVisible = true;
  ...
  public function compileGen(&$o, $niso)
  {
  ...
  public function getNisoForGroups($mediaId, $resize = false)
  {
    ...
    $docstruProxy = pinax_ObjectFactory::createObject('metafad.mag.models.proxy.DocStruProxy');
    ...
    {
      $nisoResize = file_get_contents($damService->resizeInfoLocal($mediaId, 'original', __Config::get('metafad.dam.resizeStreamS')));
