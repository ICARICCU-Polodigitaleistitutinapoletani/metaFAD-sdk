<?php
class metafad_mag_services_ImportMag extends PinaxObject
{
    ...
    /** var metafad_mag_models_proxy_DocStruProxyInterface */
    private $docStruProxy;
    /** var metafad_mag_services_ImportMediaInterface */
    private $dam;
    /** var metafad_mag_services_EventInterface */
    private $event;
    ...
    public function __construct(
        metafad_mag_models_proxy_DocStruProxyInterface $docStruProxy,
        metafad_dam_services_ImportMediaInterface $dam,
        metafad_mag_services_EventInterface $event)
    {
    ...
    public function importFile($file, $docStruProxy, $imageSizePriority, $checkExist,$getInventoryFromName=false, $overwrite=false)
    {
        ...
        try {
          ...
          $this->createStruMag($file,$rootId['idMag']);
            ...
            $application->executeCommand('metafad.mag.controllers.Delete', $rootId['idMag'], 'metafad.mag.models.Model', false );
        }
    ...
    private function createRootNode($xml, $docStruProxy, $checkExist,$getInventoryFromName=false,$file=null,$overwrite=false)
    {
        ...
        if ($overwrite) {
            if ($it->count()) {
                ...
                $magProxy->delete($ar->getId(), 'metafad.strumag.models.Model');
            }
        } else if ($checkExist) {
            if ($it->count()) {
                return false;
        ...
        $decodeData->__model = 'metafad.mag.models.Model';
    ...
    private function readRootData($xml,$getInventoryFromName=false,$file=null)
    {
        ...
        if($bibNode) {
            ...
            if(!$this->checkIfVoid(($shelfmark))) {
              $holding->BIB_holdings_shelfmark = array($shelfmark);
            }
            if(!$this->checkIfVoid($holding)) {
        ...
        if($data->BIB_dc_identifier_index)
        {
          ...
          {
            $link = pinax_ObjectFactory::createModelIterator($this->formType)
          ...
          {
            $link = pinax_ObjectFactory::createModel($this->formType);
    ...
    private function setBaseInfo($xml)
    {
      $data = new StdClass;
    ...
    // {
    //     $helper = pinax_ObjectFactory::createObject('chviewer.modules.docstru.helpers.TypeInstanceHelper');
    //     $type = $helper->getInstanceType();
    //     $it = pinax_ObjectFactory::createModelIterator($type['model'])
    ...
    private function createStruMag($file,$rootId)
    {
      ...
      $mag = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Model')
    ...
    private function setStartStop($elements,$key)
    {
      foreach ($elements as $e) {
        if($e->resource == 'img' || is_null($e->resource))
        {
    ...
    public function linkMedia($model, $externalID='')
    {
      ...
      $mediaLinkHelper = pinax_ObjectFactory::createObject('metafad.mag.helpers.MediaLinkHelper');
      ...
      if($model == 'metafad.sbn.modules.sbnunimarc.model.Model'){
        ...
        $mediaLinkHelper->linkToArchive($model,$identifier,$this->fileSequence['img'],$this->linkedStru, $externalID);
      }
      else{
    ...
    public function checkIfVoid($object) {
