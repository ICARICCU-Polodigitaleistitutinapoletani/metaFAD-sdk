Pinax.oop.declare("pinax.FormEdit.FormEditRepeatMediaGroup", {
    ...
    initialize: function (element, pinaxOpt, form, addBtnId, idParent) {
        ...
        if (!this.noAddRowButton && !this.readOnly) {
            var label = this.customAddRowLabel ? this.customAddRowLabel : PinaxLocale.Repeater.addRecord;
            this.$element.append('<div class="GFEFooter"><div class="GFEButtonContainer center"><div id="'+addBtnId+'-addRowBtn" class="btn GFEAddRow"><i class="fa fa-plus"></i> '+label+'</div></div><div class="GFEStatusContainer">' + PinaxLocale.Repeater.minRecords + this.minRec + (this.maxRec ? (PinaxLocale.Repeater.maxRecords + this.maxRec) : '') + '</div><div class="GFESideClearer"></div></div>');
        } else {
        ...
        if(!this.readOnly)
        {
          if (this.isCollapsable) {
              ...
              var rowhandler = this.sortable ? '<span class="GFERowHandler hide"><img width="16" height="38" title="' + PinaxLocale.Repeater.drag + '" alt="' + PinaxLocale.Repeater.drag + '" src="./application/templates/images/dragHandler.gif"></span>' : '';
                $rowContainer
                ...
                .children('.GFERowExpanded')
                    .append('<div class="GFERowButtonContainer"><input type="button" value="' + PinaxLocale.Repeater.confirm + '" class="btn btn-primary GFERowDoCollapse  GFERowDoConfirm">&nbsp;<input type="button" value="' + PinaxLocale.Repeater.cancel + '" class="btn GFERowDoCollapse"></div>')
                    .hide();
          }
          else {
              var rowhandler = this.sortable ? '<span class="GFERowHandler GFERowHandlerExpanded"><img width="16" style="height:100%" title="' + PinaxLocale.Repeater.drag + '" widalt="' + PinaxLocale.Repeater.drag + '" src="./application/templates/images/dragHandler.gif"></span>' : '';
              $rowContainer
                  .append(rowhandler + '<img width="16" height="16" class="icon GFERowDelete GFERightIcon" title="' + PinaxLocale.Repeater.remove + '" alt="' + PinaxLocale.Repeater.remove + '" src="./application/templates/images/icon_delete.gif">')
          }
        }
        ...
        if (!this.noAddRowButton && !this.readOnly && !this.noEmptyMessage) {
            $fieldSet.prepend('<div class="GFEEmptyMessage">' + (this.minRec ? PinaxLocale.Repeater.noRecordEntered1 + this.minRec + PinaxLocale.Repeater.noRecordEntered2 : PinaxLocale.Repeater.clickAddRecordButton) + '</div>');
    ...
    addDeleteHandler : function(containerId) {
        ...
        $('#'+containerId+' div.GFERowDelete').on('click', function () {
            ...
            if ($rows.length == 0) {
                //alert(PinaxLocale.Repeater.minRecordMsg + self.minRec);
                return;
            }
