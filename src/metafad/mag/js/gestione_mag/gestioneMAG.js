$(document).ready(function(){
  $('#linkedFormType').on('change',function(){
    if($(this).val() !== '')
    {
      $('.linkedFormContainer').removeClass('hide');
    }
    else
    {
      $('.linkedFormContainer').addClass('hide');
    }
  });
  ...
  var level = $('#BIB_level').val();
  $('#BIB_piece').children('.form-group').each(function(){
    $(this).addClass('hide');
  });
  changeVisibilityPiece(level);
  ...
  $('#file').on('click','.deleteMediaButton',function(){
    ...
    {
      $.ajax({
        url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.DeleteSingleMedia',
        type: 'POST',
        data: {
          id: idMedia
        },
  ...
  $('#list_options').on('change',function(){
    var type = $(this).val();
    $.ajax({
      url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.SetRelationType',
      type: 'POST',
      data: {
        type: type,
      },
  ...
  $('.btn-dam').on('click',function(){
    ...
    $.ajax({
      url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.CreateMediaFromDAM',
      type: 'POST',
      data: {
        media: media,
        id: id
  ...
  $('#showElements').on('click','.js-actionStru',function(){
    ...
    if(type == 'saveAll')
    {
      ...
      if(option == 0 || option == 2)
      {
        var importAll = true;
        $.ajax({
          url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.CreateMedia',
          type: 'POST',
          data: {
            type: type,
            stru: stru,
      ...
      {
        $.ajax({
          url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.CreateLogicalStru',
          type: 'POST',
          data: {
            type: type,
            stru: stru,
    ...
    else if(type == 'deleteAll')
    {
      ...
      {
        $.ajax({
          url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.DeleteMedia',
          type: 'POST',
          data: {
            id: id,
            option: option
    ...
    else if(type == 'saveCheckedMedia')
    {
      ...
      {
        $.ajax({
          url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.CreateMediaFromList',
          type: 'POST',
          data: {
            stru: stru,
            key: key,
  ...
  $('.js-element').on('click',function(){
    ...
    $.ajax({
      url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.GetElement',
      type: 'POST',
      data: {
        start: start,
        stop: stop,
  ...
  $('input[name="linkedStru"]').on('change',function(){
    ...
    $.ajax({
      url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.StruFormat',
      type: 'POST',
      data: {
        value: value
      },
    ...
    if(value)
    {
      //TODO gestione riempimento automatico dc:relation
      // $.ajax({
      //   url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.CheckMag',
      //   type: 'POST',
      //   data: {
      //     id: value.id
      //   },
      ...
      //   },
      //   complete: function(data){
      //     var identifier = data.responseJSON.result;
      //     $('#BIB_dc_relation-addRowBtn').click();
      //     $('input[name="BIB_dc_relation_value"]').last().val(identifier);
      //   }
      // });
    }
  });
...
$(document).on('change','#linkedForm',function(){
  ...
  {
    $.ajax({
      url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.GetBibFromSBN',
      type: 'POST',
      data: {
        bid: bid
      },
  ...
  {
    $.ajax({
      url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.GetMagFromRecord',
      type: 'POST',
      data: {
        id: bid
      },
...
function populateFields(result)
{
    ...
    $.each(result, function(d, value) {
        var obj = $('#'+d).data('instance');
        obj.setValue(value);
        countElements++;
    });
...
{
  $.ajax({
    url: Pinax.ajaxUrl + '&controllerName=metafad.mag.controllers.ajax.RefreshLinkedImages',
    type: 'POST',
    data: {
    },
    success: function (data, textStatus, jqXHR) {
...
function changeVisibilityPiece(level)
{
  if(level === 'm')
  {
    $('#BIB_piece').removeClass('hide');
    $('#BIB_piece').find('input[data-level="m"]').each(function(){
      $(this).closest('.form-group').removeClass('hide');
    });
    $('#BIB_piece').find('input[data-level="s"]').each(function(){
