...
class metafad_mag_models_proxy_MagRelationProxy extends PinaxObject
{
  ...
  {
    $search = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Relations')
    ->where('mag_relation_FK_document_id',$data->__id)
    ->first();
    $r = ($search) ? $search : pinax_ObjectFactory::createModel('metafad.mag.models.Relations');
    ...
    {
      $ar = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Relations')
    ...
    if($parent == 0)
    {
      if($data->flagVestito == 'false')
      {
        ...
        $childrenIterator = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Relations')
        ->where('mag_relation_stru_id',$data->linkedStru['id'])
        ->where('mag_relation_FK_document_id',$data->__id,'<>');
  ...
  public function getParent($id)
  {
    ...
    $parent = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Relations')
  ...
  public function generateDCRelation($data)
  {
    ...
    if($ar)
    {
      ...
      if($ar->mag_relation_parent == 0){
        ...
        if(!$flagVestito)
        {
          $children = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Relations')
          ->where('mag_relation_parent',$ar->mag_relation_id);
          foreach ($children as $child) {
            $cl = new stdClass();
            $child = pinax_ObjectFactory::createModel('metafad.mag.models.Model');
        ...
        if(!$flagVestito)
        {
          $parentMag = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Relations')
          ->where('mag_relation_id',$ar->mag_relation_parent)->first();
