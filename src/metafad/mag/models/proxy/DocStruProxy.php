<?php
class metafad_mag_models_proxy_DocStruProxy extends pinax_mvc_core_Proxy implements metafad_mag_models_proxy_DocStruProxyInterface
{
	public function getRootNode($id)
	{
		$ar = pinax_ObjectFactory::createModel('metafad.mag.models.Docstru');
		$ar->load($id);
		return $ar;
	}
	...
	public function readContentFromNode($id)
	{
		...
		if ($ar->load($id)) {
			if ($ar->docstru_FK_document_id) {
				$arDoc = pinax_ObjectFactory::createModel('metafad.mag.models.'.ucfirst($ar->docstru_type));
	...
	public function saveContentForNode($data, $type = 'MAG')
	{
		...
		$arDoc = pinax_ObjectFactory::createModel('metafad.teca.'.$type.'.models.'.$model);
		if ($ar->docstru_FK_document_id) {
			$arDoc->load($ar->docstru_FK_document_id);
		}
	    ...
	    $arDoc->fulltext = pinaxcms_core_helpers_Fulltext::make($data, $arDoc);
	...
	public function deleteNode($id, $cleanDocuments=true, $type = 'MAG', $deleteRoot = true)
	{
		...
		if ($deleteRoot) {
			...
			$pub = pinax_ObjectFactory::createModel('metafad.teca.'.$type.'.models.Publication');
	...
	public function saveNewRoot($id,$title)
	{
		...
		$docstru = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Docstru')
							 ->where('docstru_FK_document_id',$id)
							 ->first();
		if(!$docstru)
		{
			$docstru = pinax_ObjectFactory::createModel('metafad.mag.models.Docstru');
	...
	public function saveNewMedia($media,$rootId,$sequence,$modelType = 'MAG')
	{
		...
		$check = pinax_ObjectFactory::createModelIterator('metafad.teca.'. $modelType .'.models.'.$type)
		...
		if(!$check)
		{
			...
			$mediaModel = pinax_ObjectFactory::createModel('metafad.teca.'. $modelType .'.models.'.$type);
	...
	public function getFirstImage($id)
	{
		...
		if($rootId)
		{
			$firstImage = pinax_ObjectFactory::createModelIterator('metafad.mag.models.Img')
	...
	public function createAltImgSection($mediaModel)
	{
		...
		$content = file_get_contents($damService->resizeStreamUrlLocal($file->mediaId, 'original',__Config::get('metafad.dam.resizeStreamS') ));
		$o->altimg_md5 = md5($content);
		$o->altimg_filesize = mb_strlen($content, '8bit');

		//Recupero NISO dello stream resized
		$nisoResize = json_decode(file_get_contents($damService->resizeInfoLocal($file->mediaId, 'original', __Config::get('metafad.dam.resizeStreamS'))));
