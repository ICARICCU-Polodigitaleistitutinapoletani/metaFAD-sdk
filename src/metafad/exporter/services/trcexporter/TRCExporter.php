<?php
set_time_limit( 0 );
class metafad_exporter_services_trcexporter_TRCExporter extends PinaxObject{
    ...
    function processSimpleField($element, $work)
    {
        ...
        if(!$value)
        {
            ...
            else if(is_object($work))
            {
                $value = $work->$key;
            }
        }

        return (!empty($value) || $value === '0') ? metafad_exporter_services_trcexporter_ICCDTrc::line($key, rtrim($value)) : '';
    }
    ...
    function processSimpleContainer($element, $work)
    {
        ...
        if ($this->isObjectNotEmpty($work)) {
            $trc = metafad_exporter_services_trcexporter_ICCDTrc::line($key);
        ...
        if ($work->$key != null)
            foreach ($work->$key as $r) {
                ...
                if (!empty($value)) {
                    ...
                    $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line($key, rtrim($value));
                }
            }
    ...
    function processAUT($element, $work)
    {
        ...
        if ((get_class($work)=='stdClass' && property_exists($work, 'AUT')) || property_exists($work, 'AUT')) {
            $auts = $work->AUT;
            if (empty($auts))
                return '';
            ...
            foreach ($auts as $aut) {
                $fields = array();
                $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line('AUT');
                //echo"<br> >>>>>";
                  ...
                  //echo"<br>$key";
                    if ($key == '__AUT') {
                        if ($aut->$key->id != NULL) {
                            ...
                            $w = pinax_ObjectFactory::createModel($moduleName . '.models.Model');
                            $w->load($aut->$key->id);
                            ...
                            //     }
                              ...
                              foreach($element['children'] as $autabil){
                                ...
                                if($arr[$autabil['name']]!=''){
                                  $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line($autabil['name'], $arr[$autabil['name']]);
                                  $fields[] = $autabil['name'];
                                  //echo "*";
                                }else{
                                      ...
                                      if (is_array($aut->{$autabil['name']}) && !in_array($autabil['name'], $fields)) {
                                          foreach ($aut->{$autabil['name']} as $r) {
                                              foreach ($r as $rkey => $rvalue) {
                                                  if (!empty($rvalue)) {
                                                      $k = explode('-', $rkey);
                                                      $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line($k[0], $rvalue);
                                                  }
                                              }
                                          }
                                          $fields[] = $autabil['name'];
                                      } elseif (!empty($aut->{$autabil['name']}) and !in_array($autabil['name'], $fields)) {
                                          $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line($autabil['name'], $aut->{$autabil['name']});
                                          $fields[] = $autabil['name'];
                                      }
                                  }
                        ...
                        if (is_array($value) && !in_array($key, $fields)) {
                            ...
                            $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line($key, $value);
                            $fields[] = $key;
                        }
                    }
                }
    ...
    function processBIB($element, $work)
    {
        ...
        if ((get_class($work)=='stdClass' && property_exists($work, 'BIB')) || $work->fieldExists('BIB')) {
            ...
            foreach ($bibs as $bib) {
                $fields = array();
                $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line('BIB');
                        ...
                        if ($bib->$key->id != NULL) {
                            ...
                            //     }
                              ...
                              foreach($element['children'] as $bibabil){
                                if($arr[$bibabil['name']]!=''){
                                  $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line($bibabil['name'], $arr[$bibabil['name']]);
                                  $fields[] = $bibabil['name'];
                                  ...
                                  if (strpos($bibabil['name'], 'BIB')!== false && $bib->{$bibabil['name']}!='') {
                                      if (is_array($bib->{$bibabil['name']}) && !in_array($bibabil['name'], $fields)) {
                                          ...
                                          $trc .= metafad_exporter_services_trcexporter_ICCDTrc::line($bibabil['name'], $bib->{$bibabil['name']});
                                          $fields[] = $bibabil['name'];
                                      }
                                  }
    ...
    {
        if (is_a($work, 'pinax_dataAccessDoctrine_ActiveRecord'))
    ...
    function getIMMFTAN($work, &$counter, &$images)
    {
    	...
    	foreach ($FTAarr as $fta) {
            ...
            if (array_key_exists('FTA-image', $fta)) {
    		    ...
    		    $imagename = str_replace(' ', '_', $img->title) . '.jpg';
    ...
    {
      $dirname=pinax_Paths::get('ROOT').'export/'.$folderName;
    	...
    	if ($type == 'AUT') {
    	   if (!file_exists($dirname . "/A" . $ESC . "AUT"))
    	       $trc = metafad_exporter_services_trcexporter_ICCDTrc::getTRCHeader($this->version, $filename, $workCount, $dirname) . $trc;
    	   ...
    	   file_put_contents($dirname . '/' . $filename, metafad_exporter_services_trcexporter_ICCDTrc::getTRCHeader($this->version, $filename, $workCount, $dirname) . $trc);
      ...
      if ($type != 'AUT' && $type != 'BIB')
          $this->zipDir(pinax_Paths::get('ROOT').'export/', $folderName);
    }
