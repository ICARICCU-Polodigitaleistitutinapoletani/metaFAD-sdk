...
class metafad_exporter_services_catexporter_CatExporter extends PinaxObject
{
    var $dirRead;
    var $dirWrite;
    ...
    {
        $this->dirRead = pinax_findClassPath('metafad/exporter/services/catexporter/input', false).'/';
        $this->dirWrite = "./input/";
    }
    ...
    {
        $estremoRemotoCodificaData = explode('-', $cronologia->estremoRemoto_codificaData);
        $estremoRecenteCodificaData = explode('-', $cronologia->estremoRecente_codificaData);

        $a = reset($estremoRemotoCodificaData);
        $b = end($estremoRecenteCodificaData);
    ...
    public function CreaCAT($idCA)
    {
        ...
        $recCA = pinax_ObjectFactory::createModel('archivi.models.ComplessoArchivistico');
    ...
    public function CreaSP($idSP)
    {
        ...
        $recSP = pinax_ObjectFactory::createModel('archivi.models.ProduttoreConservatore');
        ...
        if ($data->persona_denominazione) {
            foreach ($data->persona_denominazione as $i => $persona_denominazione) {
                $rQuery = "/san:eac-cpf/san:cpfDescription/san:identity";
                $nodo=$rXpath->query($rQuery);
                $elNE=$docRead->createElement('nameEntry');
    ...
    public function CreaSR($idSR)
    {
        ...
        $recSR = pinax_ObjectFactory::createModel('archivi.models.SchedaStrumentoRicerca');
