<?php
class metafad_importer_views_components_CustomStorageBrowser extends pinax_components_Component
{
    function init()
    {
    ...
    public function render($outputMode=NULL, $skipChilds=false)
    {
        ...
        $filter = __Request::get('filter');

// TODO localizzare l'interfaccia
        $this->_application->_rootComponent->addOutputCode( pinax_helpers_JS::JScode( 'if ( typeof(Pinax) == "undefined" ) Pinax = {}; Pinax.baseUrl ="'.PNX_HOST.'"; Pinax.ajaxUrl = "ajax.php?pageId='.$this->_application->getPageId().'&ajaxTarget='.$this->getId().'&onlyFolder='.$onlyFolder.'&customPath='.$customPath.'&enableDownload='.$enableDownload.'&filter='.$filter.'&command=";' ), 'head' );
...
$(document).ready(function() {
    ...
    function redraw( data )
    {
        ...
        html += '<h4>Posizione: $customPath'+ currentFolder+'</h4>';
...
EOD;
    ...
    function process_ajax()
    {
        ...
        if ( $command == "read" )
        {
            ...
            $result[] = array( 'type' => 'dir', 'name' => "$customPath/", 'path' => '', 'icon' => 'folder' );
