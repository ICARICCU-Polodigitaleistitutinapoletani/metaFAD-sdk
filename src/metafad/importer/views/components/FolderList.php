<?php
class metafad_importer_views_components_FolderList extends pinax_components_Input
{
    ...
    if (is_dir($folderPath)){
      ...
      foreach($fileList as $item) {
        if($item != '.' && $item != '..' && is_dir(__Config::get($pathFromConf).'/'.$item)) {
          $output .= '<option value="'.pinax_encodeOutput($item).'">'.pinax_encodeOutput($item).'</option>';
        }
      }
      $output .= "</select>";
    } else {
