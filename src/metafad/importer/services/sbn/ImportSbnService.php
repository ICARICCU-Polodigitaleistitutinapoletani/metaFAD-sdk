<?php
class metafad_importer_services_sbn_ImportSbnService extends PinaxObject
{
  ...
  public function importJsonToDb($startingDir, $modelName, $documentType, $docType, $uploadType, $startFrom = 1)
  {
        ...
        foreach (scandir($actualDir . $dir) as $k => $value) {
          if ($value != '..' && $value != '.' && $value != '.DS_Store') {
            if ($count >= $startFrom) {
              ...
              $model = pinax_ObjectFactory::createModelIterator($modelName);
  ...
  public function updateSbnDigitale($list)
  {
    ...
    foreach ($list as $id) {
      ...
      if ($firstImage) {
        ...
        $r = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $url . '?id=' . strtoupper($id) . $digitale . $idpreview, 'GET', '', 'application/json');
        $r->execute();
      } else {
  ...
  {
    $sbnProxy = pinax_ObjectFactory::createObject('metafad.sbn.modules.sbnunimarc.model.proxy.UpdateSbnProxy');
    foreach ($list as $id) {
      $it = pinax_ObjectFactory::createModelIterator('metafad.sbn.modules.sbnunimarc.model.Model');
