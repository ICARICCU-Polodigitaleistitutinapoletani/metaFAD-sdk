<?php
class metafad_importer_services_sbn_Batch extends metafad_jobmanager_service_JobService
{
  public function run()
  {
    set_time_limit(0);
    ...
    try {
      ...
      $sbnHelper = pinax_ObjectFactory::createObject('metafad.importer.services.sbn.ImportSbnService');
      ...
      $outputFolderSolr = __Config::get('metafad.'.$format.'.outputFolder.solr').'/'.$folder;
      ...
      if($uploadType !== 'dbonly')
      {
        foreach ($arrayProfile as $profile) {
          $message = $sbnHelper->importFE($folderSolr,$outputFolderSolr,$profile,$folderWebMRC,$uploadType);
