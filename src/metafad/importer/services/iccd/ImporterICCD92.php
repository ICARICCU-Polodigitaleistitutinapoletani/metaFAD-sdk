...
class metafad_importer_services_iccd_ImporterICCD92 extends PinaxObject
{
    ...
    private function logMsg($msg, $lvl = PNX_LOG_DEBUG){
    ...
    public function __construct($trcInstance, $immftanInstance, $iccdProxy, metafad_dam_services_ImportMediaInterface $dam, $modelPath, pinax_log_LogBase $logger = null, $importDir = '')
    {
        ...
        //Esistono schede 301, ma queste hanno authority file BIB 300. In questo caso tengo
        //questa variabile versionAlt di appoggio
        $this->versionAlt = ($this->version == '301') ? '300' : $this->version;
        $this->iccdFormProxy = pinax_ObjectFactory::createObject('metafad.common.models.proxy.IccdFormProxy');
        $this->uniqueIccdIdProxy = pinax_ObjectFactory::createObject('metafad.gestioneDati.boards.models.proxy.UniqueIccdIdProxy');
        $this->dam = $dam;
        $this->logger = $logger;
        $this->dir = $importDir ?: '';
    }
        ...
        if (!empty($this->records)) {
            foreach ($this->records as $data) {
                ...
                if ($this->immftan != NULL) {
                    $this->logMsg("Scheda $id: Avvio importazione/associazione immagini", PNX_LOG_INFO);
                ...
                $this->logMsg("Scheda $id: Ricerca e associazione AUT/BIB in corso", PNX_LOG_INFO);
                ...
                $this->logMsg("Scheda $id: Salvataggio scheda in corso...", PNX_LOG_INFO);
                $data->__id = 0;
                $data->__model = $this->modelPath;
                $module = pinax_Modules::getModule($this->trc->moduleName);
    ...
    {
        $modelName = 'AUT' . $this->versionAlt . '.models.Model';
        ...
        foreach ($AUTs as $aut) {
            ...
            if ((is_object($aut) || is_array($aut)) && !empty($aut)) {
                ...
                if ($autCopy->AUTH) {
                    ...
                    $it = pinax_ObjectFactory::createModelIterator($modelName)
    ...
    {
        $modelName = 'BIB' . $this->versionAlt . '.models.Model';
