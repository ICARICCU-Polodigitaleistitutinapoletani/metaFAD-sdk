<?php
class metafad_importer_services_iccd_ImmftanFromDB implements metafad_importer_services_iccd_ImmftanInterface
{
    private $values;
        ...
        foreach ($records as $work)
            if (array_key_exists('FTA', $work)) {
                foreach ($work['FTA'] as $fta) {
                    ...
                    $fullpath = __Config::get('metafad.importer.services.iccd.ImmftanFromDB.folder') . $fta['fotodir'] . $fta['fotoname'];
