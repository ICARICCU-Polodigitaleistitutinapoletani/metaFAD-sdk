<?php
class metafad_importer_services_iccd_Immftan implements metafad_importer_services_iccd_ImmftanInterface
{
    private $values;
    ...
    public function getSomePossibilities($string){
        return array(
            $string,
            strtolower($string),
            strtoupper($string),
    ...
    public function tryOtherNames($fullname){
    ...
    private function parseLine($line)
    {
        ...
        elseif (preg_match('/^([0-9]+),(.+?),FTAN:(.+?),NCTR:([0-9]+),NCTN:([0-9]+),,/', $line, $matches))
            $key = implode('-', array($matches[3], $matches[4], $matches[5], 'ncts', 'rvel'));
        elseif (preg_match('/^([0-9]+),(.+?),FTAN:(.+?),NCTR:([0-9]+),NCTN:([0-9]+),NCTS:(.+?),/', $line, $matches))
            $key = implode('-', array($matches[3], $matches[4], $matches[5], 'ncts' . $matches[6], 'rvel'));
