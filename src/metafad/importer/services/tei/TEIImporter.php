<?php
class metafad_importer_services_tei_TEIImporter extends PinaxObject{
...
function insManoscritto($nodoManoscr){
  $params = new StdClass();
  $params->schemafile=pinax_Paths::get('ROOT')."application/classes/metafad/common/importer/jsonSchemas/TEI/manoscritto_schema.json";
  $params->namespaces = array('x' => 'http://www.tei-c.org/ns/1.0');

  $input= new StdClass();
  $input->domElement=$nodoManoscr;

  $mainRunner=pinax_ObjectFactory::createObject("metafad.common.importer.MainRunner");
  $XmlToJson=pinax_ObjectFactory::createObject("metafad.common.importer.operations.XmlToJson", $params, $mainRunner);
  $retXmlToJson=$XmlToJson->execute($input);

  $SaveTEI=pinax_ObjectFactory::createObject("metafad.common.importer.operations.SaveTEI", $params, $mainRunner);
...
function insUT($nodoUT){
  $params = new StdClass();
  $params->schemafile=pinax_Paths::get('ROOT')."application/classes/metafad/common/importer/jsonSchemas/TEI/unitaTestuale_schema.json";
  $params->namespaces = array('x' => 'http://www.tei-c.org/ns/1.0');
...
function insUC($nodoUC){
  $params = new StdClass();
  $params->schemafile=pinax_Paths::get('ROOT')."application/classes/metafad/common/importer/jsonSchemas/TEI/unitaCodicologica_schema.json";
  $params->namespaces = array('x' => 'http://www.tei-c.org/ns/1.0');
